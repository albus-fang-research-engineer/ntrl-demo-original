...
for label, d in DIRS.items():
    if label == OURS_LABEL:
        # New format: no successes.npy, missing path_XXX.npy = non-convergent,
        # all existing paths are collision-free.
        indices = []
        for p in sorted(Path(d).glob('path_*.npy')):
            stem = p.stem  # 'path_001' or 'path_001_collision'
    ...
    lengths = {}
    for i in indices:
        path_rad = np.load(f'{d}/path_{int(i):03d}.npy')
        lengths[int(i)] = {
